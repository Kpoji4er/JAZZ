...
Тяжёлый `jazz-maps/Maps/` по умолчанию не обходится. Его содержимое проверяется только с `-IncludeMapsContent` по прямому указанию на карту, сектор или map patch; metadata и корневые generated layers пакета проверяются всегда.
...
Без `-Package` аудитор охватывает комплект ниже. Для локальной транзакции выбирать пакет-владелец через `-Package`; межпакетные ссылки проверять при rename/удалении/public ID и другом реальном impact.
