...
Обычный режим аудитора печатает найденные расхождения, но сохраняет baseline-совместимость. `-Strict` завершает работу кодом `2`, если обнаружено хотя бы одно расхождение; отсутствие core-файлов или невозможность разбора завершается кодом `1`.
