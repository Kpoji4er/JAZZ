...
- Для полной замены vanilla-класса companion сохраняет исходные class name/ID и объявляет `UndefineClass('<Id>')` непосредственно перед `DefineClass.<Id> = { ... }`; отсутствие любой строки пары делает generated-транзакцию неполной.
