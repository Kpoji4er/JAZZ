...
-- Capture → wepicon9_<hash>.raw.png → jazz_wepicon_hook.cmd (via ImportImage/AsyncExec) → .png
...
function JazzWeaponIcon_IsStockConfig(weapon)
	if not weapon or not weapon.ComponentSlots then
		return true
	end
...
local function SetupSideCamera(vis)
	...
	local cam_pos = center + point(0, MulDivRound(extent, 1100, 1000), MulDivRound(extent, 160, 1000))
...
local function PlaceBakeBackdrop(base_pos)
	...
	if bg.SetSIModulation then
		bg:SetSIModulation(255)
	end
	bg:SetAxis(point(0, -4096, 4096))
	bg:SetAngle(180 * 60)
	bg:SetPos(base_pos + point(0, 2000, 0))
...
local function DoDedicatedBake(weapon)
	if not JazzWeaponIcon_IsFirearm(weapon) or JazzWeaponIcon_IsStockConfig(weapon) then
		return false, "stock-or-nonfirearm"
	end
	...
	add_light(point(MulDivRound(guim, 8, 10), MulDivRound(guim, 14, 10), MulDivRound(guim, 6, 10)), 55)
	add_light(point(-MulDivRound(guim, 7, 10), MulDivRound(guim, 9, 10), MulDivRound(guim, 4, 10)), 35)
	...
	local ok, err = pcall(function()
		...
		if src_box then
			...
			capture_opts.width = 512
			capture_opts.height = 256
			print("[JAZZ-UI-001] capture src box", tostring(src_box))
		end
...
function JazzWeaponIcon_QueueDedicatedBake(weapon)
	if not weapon or JazzWeaponIcon_IsStockConfig(weapon) then
		return
	end
	local fp = JazzWeaponIcon_Fingerprint(weapon)
	...
	CreateRealTimeThread(function()
		...
		Sleep(200)
		local ok, success, detail = pcall(DoDedicatedBake, weapon)
		Queued[fp] = nil
		Busy = false
...
JazzWeaponIcon_WipeMemoryCache()
...
function OnMsg.WeaponModifiedSuccess(weapon, ...)
	if not weapon then
		return
	end
