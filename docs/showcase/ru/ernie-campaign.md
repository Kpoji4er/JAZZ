...
На крупных стычках высадки (**M1**) ход союзных повстанцев укорочен: меньше расчётов позиции/укрытия и ускоренное проигрывание их действий при авто-Fast Forward.
...
Рядом на острове, но **без** Ernie-тега в ModItemSector: `K2` (Старая ретрансляционная башня), `I3`/`I4` (дорога к маяку), `M3` (водопад), `K3`/`L3`/`L4` (квест виллы), `I7_Underground`, `L6_Underground`.
...
После освобождения деревни при вражеском форте — **квестовая** sat-атака с форта на деревню (~16 ч). На maps: `ErnieCounterAttack` ~**30** (РПГ + 2 гренадёра T1 + миномёт). На **Vanilla Maps (NoMaps):** `ErnieCounterAttack_NoMaps` ~**20**, **без миномёта**. Не общий AI «Возмездие». Обычные ванильные периодические вылазки с managed-форта не включаются.
| `ErnieSideQuests` | *(без имени)* | да |
| `ErnieSideQuests_WorldFlip` | *(без имени)* | да |
| `RescueTeam` | Мы в спасатели не нанимались | нет |
| `RebelsSavior` | Снабжение для повстанцев | нет |
...
| `Jazz_DeadPigs` | Свинорез |
| `Jazz_Alkatraz` | Зачистить бункер |

## Исправленные квестовые цепочки

- В K5 повстанцы принимают только полный комплект: **4 Zastava M76 + 4 Medkit**. После сдачи у палаток появляется Берриман Сил — пилот-контрабандист, которого можно бесплатно взять в отряд.
- Спасение пленника на пирсе засчитывается только если он выжил; после этого нужно вернуться к сержанту.
- Квест доктора требует отдельно стабилизировать **трёх** раненых, а не одного вместо всех.
- После принятия «Свинореза» в K6 к бою присоединяются **четыре** союзника Балумбы.
- Помощники повстанцев в K4 и M4 больше не исчезают из-за неверной секторной привязки.
- Линия Германа ведёт на **J7**, а зачистка бункера — в **L6 Underground** с докладом в L1. Схроны у входа **L6** и в подземке — островной тир (патроны T1, тайник разведки, дробовик), не шахтные алмазы материка.
