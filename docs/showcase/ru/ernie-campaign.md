...
После освобождения деревни при вражеском форте — **квестовая** sat-атака с форта на деревню (~16 ч). Состав `ErnieCounterAttack`: ~**37** тел, **1** РПГ, **2** гренадёра T1 (метатели) + **1** миномёт (не общий AI «Возмездие»). Под Global AI / Vanilla Maps это снова работает; обычные ванильные периодические вылазки с managed-форта не включаются.
