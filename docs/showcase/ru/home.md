...
- [Тактический ИИ](tactical-ai.md) — роли, ход врага, дым, ночь, медик
...
В обычном UI **нет точного % попадания**: силу эффекта показывают `+` / `−` (проценты — с debug tools). Для гранат/GL цвет зоны поражения и дуги броска, а также индикатор mishap отражают риск провала (см. [Бой и точность](combat-and-accuracy.md)).
