...
- Bleeding tiers **3 / 6 / 12** HP/stack/turn. Hotbar: **Field Bandage** (one use spends **one bandage per bleed stack**, each step −1 tier) and kit **Bandage**. **Small** (stack **5**, Medical **30**): full bleed clear + healing on a **light** trauma. **Medium** (stack **10**, Medical **50**, +50% heal): same + healing on **medium or light**. **Large** (stack **15**, Medical **80**, +100% heal): full bleed clear + healing on **any** trauma. Any medkit clears **Pain** and **wound infection** and can rally the downed. “Healing” means the `jazz_healing` flag (not instant trauma clear). Starting hire loot: Medical **&lt; 20** → bandages only; kits by skill (AME Small only).
