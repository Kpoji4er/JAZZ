...
- Bleeding tiers **3 / 6 / 12** HP/stack/turn. Hotbar: **Field Bandage** and kit **Bandage**. **Small** (stack **5**, Medical **30**): full bleed clear + healing on a **light** trauma. **Medium** (stack **10**, Medical **50**, +50% heal): same + healing on **medium or light**. **Large** (stack **15**, Medical **80**, +100% heal): full bleed clear + healing on **any** trauma. Any medkit clears **Pain** and **wound infection** and can rally the downed. “Healing” means the `jazz_healing` flag (not instant trauma clear). Starting hire loot: Medical **&lt; 20** → bandages only; kits by skill (AME Small only).
...
- **Armor weight:** reduces Free Move (status stacks = Free Move AP lost). A heavy kit may also cut up to **2** start-of-turn AP. Strength / Ironclad mitigate; **KillingWind** ignores. At **6+** weight stacks, the first move this turn adds **+1 Pain** (at most one stack per turn from weight).
