...
- Bleeding has three tiers (**3 / 6 / 12** HP per stack per turn). Hotbar: **Field Bandage** (`JazzBandage`, ~1 AP) drops the worst stack by **one tier** (no Medical; stacks to **30**). **Bandage** uses IFAK (stack **5**, Medical **30**) or Medkit (stack **3**, Medical **50**). Both restore HP and remove **all** bleeding; Medkit heals **50%** more. One use spends one item. The rollover shows the threshold and turns it into a warning when the current merc falls short.
- **Heavy** bleed from hits comes from **expanding** ammo (JHP).
- **Pain** cuts AP and accuracy (−1 stack/turn; **clears fully when combat ends**): each solid damaging hit adds **+1** Pain (grazing scratches do not). **Morphine** immediately **clears Pain** and **blocks new stacks** while Analgesia lasts; it also refunds, once, the AP Pain already withheld at the start of the current turn. It **can rally a downed ally** (like a medkit — no HP heal) and does not stop bleeding or trauma.
...
- Frag / HE / flashbang: **guaranteed concussion** on blast-hit units, plus a chance of **zone trauma**. In the **center blast ring** only, a strong blast can also **knock units back** one tile from the epicenter (mercs included) — **Strength + Health** skill check vs **pre-armor** damage; outer ring has no knockback; already prone units stay put. Smoke / gas / Molotov use their own packages, not concussion/knockback.
