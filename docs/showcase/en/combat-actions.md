...
| MG Setup | Same distance-scaled cone as Overwatch (closer is wider). Does not fire by itself. A mapped stationary MG stays under control after loading a save (no remount needed). Every mapped gun keeps a long cone at that gun's max range and a **45°** sweep, not Overwatch's close-in minimum. |
