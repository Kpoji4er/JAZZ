...
| `GloryHog` | PierreMerc | Machete **Charge** (non-straight) +**15** Grit; active **Recruit** — one button, pick a visible enemy from the HUD list (single target auto-fires) → AI ally / combat (not bosses) |
...
**Batch5 HARD/satellite:** Rothman (loyalty-scaled mine income), Miguel (aura 30 Will/CTH), Ira (+20 primary on militia she trains), **Barry** (`DesignerExplosives`): every **168 h** produces **2× Shaped Charge**; craft via Craft Explosives; **−30% Parts** on CraftAmmo/CraftExplosives recipes and consume while Barry is in the sector (assigned or Idle; TNT/powder unchanged). Meat Will→Grit, Carlos detection/Hidden, Cord city repair, Conrad Leadership≥90 as trainer. **Igor** (`Nazdarovya`): drink 2 AP, CD on kill — heal/Pain/Drunk stacks. **Thor** (`NaturalHealing`): joints every 48 h; sat squad +15% trauma/burn/HP debt recovery (not infection); bandage restores 20–25 Will. Soft: Biff trooper economy, Livewire money op (ECON-001).
