...
| `JazzArmor_FlakM69` | Torso | `EquipmentMale_FlackVest` | vanilla | C1 `78,88,52`; mock PASS |
| `JazzArmor_IBALight` | Torso | `EquipmentMale_InterceptorVest_01` | vanilla | C1/C2/C3 из таблицы; mock PASS |
| `JazzArmor_IBA` | Torso | `EquipmentMale_InterceptorVest_02` | vanilla | C1/C2/C3 из таблицы; mock PASS |
| `JazzArmor_IBAFull` | Torso | `EquipmentMale_InterceptorVest_02` | vanilla | более тёмный C1/C2; mock PASS |
| Mapped Head (`UniformCap` … `GuardianHelmHeavy`) | Head | `FactionMale_Hat_*` / `EquipmentMale_WW2Helmet` / `JungleCamp_GraveyardHelmet_02` / `Construction_Helmet_01` | vanilla | `parts.Hat` + Hide Hair; mock PASS |

Тестовые `JAZZ_Legion_ArmorTest*` (`jazz-units`) используют `LegionGoon`, соответствующий предмет в Torso или Head, MP40 и 120 патронов `JAZZ_AMMO_9x19_FMJ`; магазин заряжен. В редакторе группа `JAZZ Tests`, display name переиспользует «Новобранец». Юниты не включены в боевые pools. Для остальных тел/одежды требуется подгонка; универсальное отсутствие пересечений не гарантируется.

## Архив исходной таблицы (до 2026-09-15)

Mapped Torso (Flak/IBA) и mapped Head из этой таблицы активированы 2026-09-19 для `JAZZ_Legion_` Male. Остальное (todo, очки/NVG, Female, Scale/Offset, мод-опция) по-прежнему архив.
...
Текущая кираса v5 использует интерполяцию весов официального тела для спины, лямок и наплечника, включая ключицы/twist; грудной объём сохранён. Попытка дополнительной загрузки тестовой копии предшествовала Access violation; новые runtime-загрузки прекращены. Последующая очередь и реальные референсы: [legion-armor-production.md](legion-armor-production.md).