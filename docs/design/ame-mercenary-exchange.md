...
- Appearance sources: **unique clone per slot** `JAZZ_AME_NN`; Legion clothing from **jazz-units handcrafted `Legion*`** (canon; not vanilla `Legion_*`) + Rebels / GrandChien / keep; Irregulars lean jazz Legion*; map [`ame-appearance-map.json`](ame-appearance-map.json); policy [`ame-appearance-assets.md`](ame-appearance-assets.md) (blue accent, NPC female hair, hat≠hair, strip hard helmets)
