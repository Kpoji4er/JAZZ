"""Insert MagNormal Visual ApplyTo=UMP45 → Magazine/UMP45_Mag25.png."""
...
def main() -> None:
    ...
    if 'ApplyTo = "UMP45"' in body and "UMP45_Mag25" in body:
        print("already")
    else:
        for key in ('ApplyTo = "UZI"', 'ApplyTo = "TMP"', 'ApplyTo = "Thompson"', 'ApplyTo = "SpectreM4"'):
            needle = (
                "\t\t\t\t\t\t\tPlaceObj('WeaponComponentVisual', {\n"
                f"\t\t\t\t\t\t\t\t{key},"
            )
