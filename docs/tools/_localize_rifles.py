"""Add rifle translation memory; run canonical audit/export after integration.

python docs/tools/_localize_rifles.py --game-csv <Game.csv> --build <build> --texts <manifest.json>
...
with path.open('a',encoding='utf-8',newline='') as f:
    ...
    for ident,ru,en in TEXTS.values():
        if ru not in seen:
            n+=1
            writer.writerow({'N':n,'AnchorID':ident,'SourceText':ru,'English':en,'Notes':'manual-translation; rifle'})
            seen.add(ru)
shutil.copy2(ROOT/'Localization/Strings.csv',build/'Strings.csv')
subprocess.run(['powershell','-NoProfile','-File',str(ROOT/'docs/tools/_export_rifle_localization.ps1'),
    '-GameCsv',str(Path(args.game_csv).resolve()),'-Build',str(build),'-TextManifest',str(Path(args.texts).resolve())],check=True)
...
for relative,staged in [('Russian.csv','Russian.csv'),('English.csv','English.csv'),('Localization/Strings.csv','Strings.csv')]:
    ...
    for row in selected:
        assert row['ID'] not in existing or row==existing[row['ID']], 'Existing rifle translation differs'
...
print('Installed only the selected canonical rifle rows from the same RU/EN catalog snapshot; all prior runtime rows preserved.')
