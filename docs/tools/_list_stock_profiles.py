...
for b in placeobj_blocks(t, "ModItemWeaponComponent"):
    ...
    if not (
        cid.startswith("JAZZ_Stock")
        or cid in ("JAZZ_PKMModStock", "JAZZ_UnfoldStocks")
        or "Stock" in cid
        and cid.startswith("JAZZ_")
    ):
        continue
    slot = prop(b.text, "Slot") or ""
    if slot and slot not in ("Stock", "False", ""):
        # still print if Stock in name
        pass
    ...
    print(
        f"{cid}|slot={slot}|{dn.group(1) if dn else ''}|"
