...
| `GenerateEnemySquad` | `Lua/Guardpost.lua` | — | core `Code/Guardpost.lua`, затем NoMaps wrapper | COMPAT-010: skip class-remap `DiamondRedSquad` / F5 `LegionDefenders_Balanced_Easy`; иначе `lRemapSquadId` |
| `CreateUnitData` | engine UnitData | — | NoMaps `Code/NoMaps_Autonomy.lua` wrapper | COMPAT-004 class-remap; COMPAT-010 `g_JAZZ_NoMapsSkipUnitRemap` для quest/story |
| `UnitMarker:SpawnObjects` | `Lua/ClassDefs/ClassDef-Zulu.generated.lua` | — | NoMaps wrapper | COMPAT-004 mutates generic Legion DefId; COMPAT-010 skip `LegionWaterWell` + Pierrot `conflict_ignore` |
