...
| `OnMsg.ReachSectorCenter` (crocodile) | `Lua/HotDiamonds.lua` | — | maps `Code/System_JAZZ_CrocodilePatrol.lua` (best-effort static replace + always Msg wrap) | Vanilla `for i=1,place` падает при place=nil вне G13–G14; wrap обязателен — StaticReplace alone мог пропустить HotDiamonds |
