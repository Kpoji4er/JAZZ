...
JAZZ поддерживает только последнюю опубликованную CommonLib. Перед каждым анализом сначала проверить официальный `main` и metadata, затем строить эту матрицу по найденной версии. Срез на 1 сентября 2026 года: версия 1.11, build 1060, commit `6758d82e5fccbf6bdd01b0173d2496b925982c3e`; это датированный снимок, не pin. Полная таблица символов ниже — наследник июльского аудита; после 1056 в CLib не появилось новых одноимённых пересечений с колонкой JAZZ.
...
| `MachineGunEmplacement:Update` | `Lua/Tactical/Emplacement.lua` | — | `Code/System_EmplacementAmmo.lua` (wrap) | ammo remap + runtime `target_dist` = gun MaxRange (`Jazz_EmplacementConeDist`; `updating` guard) |
| `MachineGunEmplacement:EndInteraction` | `Lua/Tactical/Emplacement.lua` | — | `Code/System_EmplacementAmmo.lua` (wrap) | cone length from gun MaxRange; placed `cone_angle` = 45° (`Jazz_EmplacementConeAngle`), not COMBAT-009 `GetOverwatchConeAngle(d)` |
| `CombatActions.Overwatch.GetMaxAimRange` | `Data/CombatAction.lua` | — | `Code/System_OR_Weapons.lua` (wrap) | skip sight clamp only for `emplacement_weapon` / `ManningEmplacement` |
