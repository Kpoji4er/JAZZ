...
Тест проверяет процентные диапазоны live/casualty crowd modifier, melee floor, medic crowd exemption (100), cover-spacing 100/55/30, единственную интеграцию crowd+cover после BiasMarker, сохранение hard XYZ dibs и отсутствие RNG/persistent/map-wide зависимостей. Он не заменяет runtime-проверку горловины, подхода медика и melee.
...
- **Grazing (JAZZ-COMBAT-002):** при низком CTH (~20%) miss→graze ~16% на дистанции ≥8; в упоре (0 клеток) ~32% (cap→50); полное укрытие даёт cover-graze ≈100%; выстрел/нож через дым **без** укрытия не форсирует graze; царапина ≈40% урона без crit/trauma; ~15% шанс лёгкой крови.
...
- **Suppression / LR / Psycho (JAZZ-COMBAT-003):** `suppressionPinned` цель с Hotblood/Shatterhand/HaveABlast не отвечает Retaliate; частичное подавление режет CTH в упор и шанс Retaliate/LR (×90/80/70/60), на pinned = 0; pinned снимает Overwatch/PinDown/Bombard (в т.ч. permanent MG) и не даёт OW-выстрел; stealth kill / Hidden не даёт float «Lightning Reaction»; Psycho после боя имеет полный Will, за ход теряет ~4 а не 8.
...
- штраф тяжёлой брони бойцу с разной силой (JAZZ-COMBAT-005: FM × Scale.AP; AP 0/1/2 по raw; STR; Ironclad −50% FM + AP ÷2; KillingWind ещё −50% FM; Pain при FM≥6 на первом Move ≤1/ход);
...
Также проверить inventory rollover, перенос между слотами, доступные UI actions, шкалу воли, AIM filters и отсутствие ссылок на отсутствующие иконки. Для динамических contexts проверить, что mutating-методы вызываются на настоящем объекте, а не на `SubContext()`. Закрытие окна должно удалить его именованные real-time threads. Pierre `Jazz_PierreRecruit`: на хотбаре **одна** кнопка Вербовки → список целей (talk); один видимый враг выполняется сразу. После вербовки нет слоя `IModeCombatAttack` — можно сразу выбирать наёмников.
...
## CI-аудит спецификаций

С JAZZ-CI-SPEC-001 шаг Validate change specifications является рекомендательным: Phase All выполняется, но его failure даёт warning и запись в summary без блокировки job. Остальные проверки остаются обязательными. Это не переводит непроверенные acceptance criteria в PASS.
