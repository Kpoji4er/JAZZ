...
Также проверить inventory rollover, перенос между слотами, доступные UI actions, шкалу воли, AIM filters и отсутствие ссылок на отсутствующие иконки. Для динамических contexts проверить, что mutating-методы вызываются на настоящем объекте, а не на `SubContext()`. Закрытие окна должно удалить его именованные real-time threads. Pierre `Jazz_PierreRecruit`: на хотбаре **одна** кнопка Вербовки (не копии иконки Charge на каждого видимого врага); клик по цели в мире; Charge (`GloryHog`) отдельно. После вербовки интерфейс возвращается в движение — можно снова выбирать наёмников (не застревать в targeting без кликов).
