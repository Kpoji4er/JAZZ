...
**Стационарный MG (`MachineGunEmplacement` / `BrowningM2HMG`):** ванильный `Update` при пересоздании ствола пишет `target_dist = MinRange`. Пока MinRange совпадал с `WeaponRange`, конус оставался длинным. После COMBAT-009 это схлопывало сектор к ~14 клеткам (50% BDR 28), а `EndInteraction` / load reseat дополнительно резали по `Overwatch:GetMaxAimRange` (зрение; ночь + prone — ещё короче). `MGRotate` на станке скрыт, короткий конус залипал. `Jazz_EmplacementConeDist` сохраняет map `target_dist` в `[MinRange, WeaponRange]` и при значении ниже минимума ставит **MaxRange**.
