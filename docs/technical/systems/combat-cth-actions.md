...
**Dump / AI targeting (JAZZ-AI-PERF-003 / PERF-004):** `AIGetAttackTargetingOptions` берёт CTH из `CalcChanceToHit`, без `GetActionResults` на каждую часть тела (M3 PickBest вис на луче body-part). Перед списком частей — один `GetLoFData` с теми же аргументами, что zone/dest score в `CombatAI` (`stuck` → нет выстрела). `AIActionDump` ставит `args.jazz_ai_dump`: `PrepareAttackArgs` и execute `GetAttackResults` не зовут `GetLoFData` **на пулю**; один prediction `GetLoFData` заполняет `hits`/`stuck`; miss — синтетический `stuck_pos`; `ProjectileFly` без vegetation `Collide`. Непробиваемый LoF — DumpFire и `BasicAttack` не стреляют. Dump в модель при командном LOS и чистом LoF; личный LOS не обязателен. Игровой Fanning и не-Dump выстрелы — ванильный пайплайн. Перед `collision.Collide` обёртка `Firearm:ProjectileFly` / `FireBullet` поднимает 2D `attack_pos`/`stuck_pos` через `SetTerrainZ` (`Jazz_EnsurePointHasZ`): стоячие юниты часто без Z, JA3Debug иначе ассертит `z != nInvalidZ`.

**LOS vs LoF:** `HasVisibilityTo` (sight: дальность, тьма, дым −70, stealth) и `GetLoFData` / cheap ray (геометрия пули) — разные проверки. Командный LOS (`HasVisibilityTo(team, target)`) даёт право выбрать модель цели; выстрел идёт только если LoF чистый. Личный LOS только бонус к score (`×1.2` в `PickBestAttack`). Движок **может** посчитать LoF без LOS (луч не знает «вижу/не вижу»), но AI обычно не зовёт его: нет team vis → нет цели-модели; dest без `CheckLOS` в DestLos cache → skip `GetLoFData` (PERF-001); Dump targeting CTH без per-part LoF, stuck-gate — один `GetLoFData`. Дом/стена чаще режут и LOS и LoF; дым режет LOS, LoF сквозь дым в JAZZ есть (без smoke-graze); за дальностью зрения team vis нет — в модель не бьют, даже если геометрический луч был бы чистым.
...
- `FireFlare`: ванильный `CombatAction` (area-aim). `UndefineClass('FlareGun')` в `WeaponClasses.lua` обязан вернуть `ValidatePos` / `GetBaseDamage` / `GetAttackResults` — иначе `IModeCombatAreaAim` падает (`ValidatePos` nil) и слот пустой. `GetDefaultAttackAction` по-прежнему обнуляет `FlareGun` (слот по умолчанию — UnarmedAttack); `FireFlare` приходит из `AvailableAttacks` через `add_weapon_attacks`. Reload HUD (`JazzHudReloadWeapons`) не фильтрует ракетницу через `GetActiveWeapons("Firearm")`.

## Стационарный пулемёт после load (JAZZ-HOTFIX-004)

Vanilla `Unit:GameInit` может вызвать `EnterEmplacement` до создания `weapon`/visual орудия. JAZZ wrap не делает `SetPos(nil)` и не падает на `obj.weapon.owner`; `LoadGame`/`EnterSector` reseat привязывает ствол, пересчитывает HUD и при `Idle` без permanent overwatch восстанавливает конус `MGTarget` на `Jazz_EmplacementConeDist` (map/`WeaponRange`, не sight). `GetActiveWeapons` при manning повторяет `Update()`, если ствол ещё nil. `ResolveDefaultFiringModeAction` / `RecalcUIActions` не падают при отсутствии оружия.
...
`Firearm:GetOverwatchConeAngle(d)` — authored `OverwatchAngle` на BDR. Ближе BDR: `authored × BDR / d`; пулемёт и ЛП ещё раз `× BDR / d` (иначе наземный клин `CreateAOETilesSector` почти не толстеет). Дальше BDR — lerp к class strip на `WeaponRange`. Clamp `[2°, 155°]`. `MinRange` = 50% BDR для всех стволов, включая M2 (это **ближайшая** посадка конуса, не длина сектора). Aim UI берёт min/max из `GetOverwatchConeParam`, не из sight-clamp `GetAimParams`. После confirm `Unit:OnOverwatchPlaced` / `OverwatchChanged` переписывает `g_Overwatch.cone_angle` и `dist` от поставленной точки — ванильный `GetAimParams` без target писал бы сырой `OverwatchAngle`.

**Стационарный MG (`MachineGunEmplacement`):** ванильный `Update` пишет `target_dist = MinRange` (после COMBAT-009 это ~50% BDR). `Jazz_EmplacementConeDist` на runtime ставит **MaxRange ствола** для каждого станка — не слайдер карты, не зрение. Угол сектора станка фиксирован **45°** (`Jazz_EmplacementConeAngle`), не COMBAT-009 полоска на MaxRange. `Overwatch.GetMaxAimRange` без зрения только на станке (`emplacement_weapon` / `ManningEmplacement`); переносной Overwatch по-прежнему Min(range, sight). `MGRotate` на станке скрыт.
