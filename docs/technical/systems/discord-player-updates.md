...
- `[skip discord]` — исключить этот коммит из player summary;
- внутри одного сообщения коммита `[skip discord]` всегда имеет приоритет над обоими публикующими маркерами;
- активны только маркеры в конце строки или на отдельной строке; упоминание `[skip discord]` в середине предложения не считается маркером;
- в смешанном push-диапазоне коммиты с `[skip discord]` исключаются из сводки, а остальные по-прежнему публикуются; весь диапазон пропускается только если все коммиты помечены `[skip discord]`.
...
Каждый репозиторий (`jazz`, `jazz_assets`, `jazz-maps`, `jazz-units`) публикует свою отдельную новость. Межпакетной агрегации нет: push в `jazz-units` не подменяет и не отменяет сводку `jazz`.
