...
Maps quest bulky items (`Jazz_Browning_MuchineGun` / `Jazz_Browning_Bench` = `MiscItem`; `JazzQuestItem_AmmoBox` / `JazzQuestItem_MinesBox` = `QuestItem`) не используют голый `object_class = "InventoryItem"` и не ставят полный PNG в `SubIcon` (ванильный `SubIcon` — маленький caliber badge на тайле; полный ящик/ствол в углу ломает layout при открытии I).

## Loot

В `jazz-units` зарегистрировано 1257 `LootDef`; они формируют loadouts и drops для 179 UnitData и squads. В `jazz-maps` есть 32 map/campaign loot definitions и размещённые контейнеры. Core добавляет классы и runtime выпадения. Ernie-контейнеры `Bunker_Shotgun` и `Drop_12gauge_Breacher` дают `JAZZ_AMMO_12gauge_Buckshot`, не cut `_12gauge_*` (`TEST.png`). Ванильные `Diamonds_Loot` и `DiamondRed_IntelStash` переопределены в `jazz-maps` (островной тир: `Jazz_Random_Ammo_Drop_T1` / `ErnieSecretStash1`), а не только ремапнуты на L6 — тот же ID подхватывают и другие маркеры (в т.ч. M5 `pSwGPnp`). **Городские лотки** (`CustomInteractable` → `UnitGrantItem`): те же ванильные `LootTableId` (`Vendor_Ammo`, `FleatownMarket_Ammo`, `PoacherCamp_Ammo`, `FosseNoire_Ammo`, …) переопределены в `jazz-maps` на живые `JAZZ_AMMO_*` (`Vendor_Ammo`: 9×19 FMJ / .44 FMJ / 7.62×51 FMJ / 7.62×39 Army / 12g Buckshot; flea `_762WP_HP`→`JAZZ_AMMO_762x39_FMJ`; poacher AP→`JAZZ_AMMO_762x39_APP`; Fosse incendiary→`JAZZ_AMMO_50BMG_APIT`). Оружейные лотки (Glock 18, MP5K, Auto-5, SVD, RPK-74, FAL, M-14) оставляют live weapon IDs. Кастомные бартеры I5 (`ubRwFgf`, `Stall_Random_762WPrandom_Parts` / `Stall_Random_12Gauge`) выдают напрямую `JAZZ_AMMO_762x39_FMJ`×40 и `JAZZ_AMMO_12gauge_Buckshot`×30, не ванильные `Drop_762WP_HP` / `Drop_12gauge_Varied_Legion`. Stall **BanterDef** — только подпись лотка, не склад. Профиль NoMaps без `jazz-maps` эти override не видит.
