...
- **`WoundInfected`** (MED-002): отдельный Debuff рядом с Heavy (Trauma* не заменяются). Untreated Heavy progress, не improve → `JazzApplyWoundInfected`. Infected progress (~8h, survive **40%**): успех снимает Infected; провал → `JazzKillMercFromInfection`. TreatWounds/`jazz_healing` на Heavy improve 100% → вход в Infected невозможен.
