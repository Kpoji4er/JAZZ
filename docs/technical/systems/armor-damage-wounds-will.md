...
- `Code/System_EnergyLadder.lua` — JAZZ-COMBAT-007/008: лестница энергии Fit→Exhausted, gradual Free Move, satellite warn/times; Legs foot-travel slow; MED-007 travel from current HitPoints vs 100 (Ribs не множат порог);
- `Code/System_Medicine_MED007.lua` — MED-007: MaxHP debt после боя (`JazzUnitSkipsTraumaMaxHpDebt` / CombatStart/End);
...
3. **Ironclad** −50% FM-штрафа от брони; **KillingWind** ещё −50% (аддитивно: оба → 0, явный `fm = 0`). Перки читаются через `JazzUnitHasPerk` (HasPerk + GetStatusEffect + `gv_UnitData`: id `KillingWind` общий с CombatAction). **Ironclad** дополнительно ÷2 AP. При `using_cumbersome` AP-штраф брони = 0. **KillingWind** выдаёт полный FreeMove с громоздким оружием/бронёй. Strength &gt; 60: `MulDivRound(STR−60,1,20)` сначала снимает AP, остаток — FM.
...
| `TraumaRibs{Light\|Medium\|Heavy}` | travel-усталость **не** ускоряют; цена — долг макс. ОЗ 10/30/60% (MED-006/007) | combat AP/Pain + `RecalcMaxHitPoints` |
| Текущие ОЗ (MED-007) | `diff = Clamp(HitPoints − **100**, −50, +25)`; порог ×(100+diff)/100. Не стат Health, не % бара. Health 90 @ 50 ОЗ = Health 50 @ 50 ОЗ. | `JazzGetHpTirednessTravelDiff` |
| HP (`GetHPAdditionalTiredTime`) | всегда **0** (чтобы не double-apply) | `SatelliteSquad.lua` |
...
**Zone-use hooks (что считается «юзом»):** Arms — `OnFirearmAttackStart` (стрельба) и `OnMsg.OnAttack` для `Melee Attack` / броска `MeleeWeapon`; Head — только `OnFirearmAttackStart`; Legs — `OnCalcMoveModifier`; Ribs/Burn — `OnCalcStartTurnAP`. Heavy-эффекты вызывают те же hooks + `OnEndTurn` → `JazzTraumaHeavyPainRamp`.
...
Квестовые NPC (ImportantNPC/villain вне отрядов игрока и не наёмники) и юниты с флагом бессмертия защищены от смерти при инфекции: провал оставляет инфекцию ещё на 16 часов. У обычных врагов и бойцов игрока смертельный исход сохраняется; бессмертие имеет приоритет. Уже погибших NPC это не воскрешает.


### JAZZ-MED-009: выбор пациентов госпиталя

HospitalTreatment.FilterAvailable использует JazzUnitNeedsTreatWounds: живой юнит с недостающим HP или нелеченной травмой доступен без Wounded. Установка выполняется при DataLoaded/ModsReloaded независимо от TreatWounds. Стоимость, лояльность, слоты и Tick пресета сохранены. Общий PatientAddHealWoundProgress переводит травму в заживление после лечебного прогресса, не удаляя её мгновенно. Изолированный Lua harness фильтра и прогресса PASS; live gosp после изменения ещё не проверен.
