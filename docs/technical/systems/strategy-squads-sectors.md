...
- **Quest punitive** `Ernie_CounterAttack` (custom id `ErnieCounterAttack`, I7→I5): EnemySquadDef `ErnieCounterAttack` — Normal base size **30** (was 37; Easy **−10** / Hard **+10** when difficulty settings exist), **1×** Sergeant, **1×** `HeavyT1_Rocketeer` + **2×** `AssaultT1_Grenadier` (T1 throwers, not hand GL) + **1×** `HeavyT3_Mortarman` + **3×** Bonemaker; fewer Shock/Raider vs prior; **2×** GMPG. Spawned via `Guardpost:SpawnEnemySquad` when `custom_quest_id` matches; `AttackWithEnemySquad` skips EnemySquadsList promote so Global AI retribution recipes stay untouched.
