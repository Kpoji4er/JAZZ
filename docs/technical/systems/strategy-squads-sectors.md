...
- Story squads: **`FortressPierre` не в `SQUAD_REMAP`** — остаётся vanilla `Pierre` + `PierreGuard`. Раньше remap → `LegionJAZZSquadT2` сажал элиту на маркер group `"Pierre"` → диалог `Pierre_1` с именем вроде «Кингбой Лайф». `FortressDefenders` по-прежнему → `LegionFortressDefenders`. Static: `docs/tools/_verify_nomaps_fortress_pierre_squad.py`.
