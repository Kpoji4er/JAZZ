...
- **Tier pulses (JAZZ-STRATEGY-026):** реальный рост `JAZZ_Legion_Tier` шлёт `Msg("JAZZ_LegionTierRaised", old, new)` → `JAZZ_LegionAIOnTierRaised`. Обычный подтир: один `supply` HQ→приоритетный живой аванпост (`MajorSupplyPriority`, затем беднее `$`). Переход на T2 (`old < 21`, `new >= 21`): на **каждый** живой незахваченный аванпост пара `supply` ($12000) + `manpower` (16); одиночный денежный импульс на этом же raise не дублируется. Пульс: `skip_global_spawn` (не ест 1/2/3), не списывает `major.money` / `major.manpower` (внешний резерв Майора), не ждёт 40% / manpower=0 / neediest, без idle reuse. Есть avoid-player путь — сразу в путь (без 12h loading); нет — спавн на HQ и hourly retry (`phase=loading`, `hold_until=now`). NewGame `11` и raise без увеличения квеста — без пульса. T3 (`25→31`) = обычный денежный подтир, не T2-burst. Save schema v3, без bump; старые сейвы получают пульс только на будущих raise.
