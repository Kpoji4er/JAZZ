...
Ролевые составы (JAZZ-STRATEGY-002, static): I7 ссылается на `LegionGlobalAI_Garrison` / `_Patrol` / `_Recon` (диапазоны 25–40 / 12–18 / 8–12, только `JAZZ_Legion_*`); QRF остаётся на `LegionJAZZSquadT2`; reinforce пока берёт garrison-list. Region `ErnieIsland` использует `LegionGlobalAI_Convoy` (15–25) для supply и shipment; Major/Retribution — `LegionJAZZSquadT3` (+ legacy `LegionHeavyTroops` в списке). Allow-list recipes + runtime generator (JAZZ-STRATEGY-007/008): `JAZZ_LegionRoleRecipes` / `JAZZ_GenerateLegionSquadComposition` собирают combat-составы; spawn combat-ролей списывает **сумму цен юнитов**. Flat Region costs — fallback если generator недоступен. Экономика (JAZZ-STRATEGY-006): outpost start/capacity **12000**/120000; Major start/capacity **120000**/1200000; POI $/h city **50**, farm **10**, mine→stock **250**; costs recon/patrol/reinforce/qrf/garrison flat fallback **8000**/18000/25000/40000/120000; supply/shipment cargo/threshold **12000**; major response flat fallback **50000**; regular cap **7**; role caps garrison 2, patrol 2, recon 1, qrf 1, reinforce 1.
