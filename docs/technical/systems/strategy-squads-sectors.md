...
- `patrol` — после каждой точки заново выбирает город, шахту, ферму или базу (нужда «всегда», пока есть цель/слот/`$`);
...
- `supply` — доставляет из `B28` `$` (`payload.money`); task UI показывает сумму; при возврате недоставленного груза восстанавливает `major.money`;
- `shipment` — везёт накопленные `$` shipment-stock из `I7` в `B28` (inventory: DiamondBriefcase на полный рейс); task UI показывает сумму;
- `major` — при Heat региона 800+ и наличии `$` создаёт отдельный тяжёлый ответ с cooldown 72 часа.

Ролевые составы (JAZZ-STRATEGY-002, static): I7 ссылается на `LegionGlobalAI_Garrison` / `_Patrol` / `_Recon` (диапазоны 25–40 / 12–18 / 8–12, только `JAZZ_Legion_*`); QRF остаётся на `LegionJAZZSquadT2`. Region `ErnieIsland` использует `LegionGlobalAI_Convoy` (15–25) для supply и shipment; Major response — `LegionJAZZSquadT3` (+ legacy `LegionHeavyTroops` в списке). Экономика (JAZZ-STRATEGY-006): outpost start/capacity **12000**/120000; Major start/capacity **120000**/1200000; POI $/h city **50**, farm **10**, mine→stock **250**; costs recon/patrol/qrf/garrison **8000**/18000/40000/120000; supply/shipment cargo/threshold **12000**; major response **50000**; regular cap 6; role caps garrison 2, patrol 2, recon 1, qrf 1. Flat role costs пока не читают `JAZZ_LegionUnitPrices`.
