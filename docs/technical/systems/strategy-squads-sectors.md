...
- `support` — малая спецгруппа (**смесь** sniper / MG / mortar на слот, **T3–T4**, размер **4–7**): только к сектору, где уже есть `garrison`/`reinforce` и соседствует player threat; отдельный `SupportCap` (default 1); иконка SUPPORT (прицел);
...
- **Tier pulses (JAZZ-STRATEGY-026 / 027):** реальный рост `JAZZ_Legion_Tier` шлёт `Msg("JAZZ_LegionTierRaised", old, new)` → `JAZZ_LegionAIOnTierRaised`. Обычный подтир: один `supply` HQ→приоритетный живой аванпост (`MajorSupplyPriority`, затем беднее `$`). Переход на T2 (`old < 21`, `new >= 21`): на **каждый** живой незахваченный аванпост пара `supply` ($12000) + `manpower` (16); одиночный денежный импульс на этом же raise не дублируется. **STRATEGY-027:** тот же T2-переход, плюс пороги **T2-3 (23)** и **T2-5 (25)** — на каждый managed-аванпост игрока (`Side` player или `owner_faction==player`) штаб шлёт `major` (возмездие, `major_response`); прыжок через несколько порогов = одна волна. Пульс логистики и возмездия: `skip_global_spawn`, не списывает казну/людей штаба, не ставит `next_response_time`. Возмездие не ждёт Heat 800. Нет живых фортов — логистика пропускается, контратаки всё равно идут. Нет HQ Легиона — контратаки skip. Есть avoid-player путь у конвоя — сразу в путь; нет — hold на HQ. `major` не обходит player-сектора. NewGame `11` без пульса. T3 (`25→31`) = денежный подтир, без новой retake-волны (порог 25 уже пройден). Save schema v3.
...
Logistics roles (`shipment`/`supply`/`tax`/`manpower`/`recruiter`/`reinforce`/`support`) планируют маршрут с `g_JAZZ_RouteAvoidPlayer` → `SectorTravelBlocked` режет player1/2 сектора. Нет обходного пути → **не спавнить** (кроме `reinforce`/`support`: спавн + hold без travel; **STRATEGY-026** tier-pulse `supply`/`manpower` тоже спавнятся и ждут путь). `patrol`/`recon`/`major` (retribution) — без avoid. Mid-route: уже идущий конвой **не** abort’ится при захвате сектора на пути (доезжает).
...
- **COMPAT-010**: class-remap **и** gear refresh **не** трогают квестовые/story спавны vanilla HotDiamonds: группа маркеров **`LegionWaterWell`** (G6 колодец), squad **`DiamondRedSquad`** (A2 шахтёры), F5 `InitialSquads` **`LegionDefenders_Balanced_Easy`** (Côte d'Azur). Живой **капитан Пьеро** получает `conflict_ignore`, чтобы ForceConflict не убивал его и не снимал операцию лодки. Static: `docs/tools/_verify_nomaps_story_quest_skip.py`.
- **COMPAT-011**: **`FortressPierre` / `Pierre` / `PierreGuard` / `NPC_Pierre`** keep-vanilla (форт **H4**). Гарнизон `FortressDefenders` → `FortressDefenders_NoMaps` как раньше. Whole-sector skip **I1 Flag Hill снят** (пустой opening был cycle RIS↔NoMaps `UnitMarker:SpawnObjects`, чинится install-once в `System_RIS_Combat.lua`). Marker remap на I1 снова обычный (`WeakFlagHill`→T1 Roughneck).
