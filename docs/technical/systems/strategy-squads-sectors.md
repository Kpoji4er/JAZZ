...
- **COMPAT-011**: на **I1 Flag Hill** (ForceConflict без InitialSquads) маркеры и live-юниты этой карты не class-ремапятся и не регеарятся. **`FortressPierre` / `Pierre` / `PierreGuard` / `NPC_Pierre`** keep-vanilla (форт **H4**). Гарнизон `FortressDefenders` → `FortressDefenders_NoMaps` как раньше.
