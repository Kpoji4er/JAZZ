...
- **PierreMerc `GloryHog`:** vanilla machete **Charge** (non-straight path via `action_id == "GloryHog"`) +**15** Grit (`Unit:GloryHogCharge`); List2 active `Jazz_PierreRecruit` (4 AP): once/combat convert a **visible** non-boss (`villain`/`ImportantNPC`) enemy to side `ally` (AI). Hotbar inject when `HasPerk(GloryHog)`. Recruit: **one** signature button → `IModeCombatAttack` click on the enemy (not `ShowCombatActionTargetChoice` / one HUD button per visible unit); after use restore movement (target `Interrupt` would otherwise leave targeting stuck). Icon `UI/Icons/Hud/talk` (Charge keeps `perk_glory_hog`).
