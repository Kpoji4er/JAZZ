...
- `Code/AiDebugLog.lua` — **JAZZ-AI-DBG-001** (loaded): mod option `AIDebugLog` (default **off**). When on, `CombatLog("short")` (Snype / combat log) writes only final dest + committed Dump/signature/melee + one abort if a planned shot failed. CTH from existing `dest_cth` / `PickBestAttack.cth`. Does not change scores, dest, or AP.
