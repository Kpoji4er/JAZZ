...
`GetCTHByAimLevels` вызывает тот же `Unit:CalcChanceToHit`, что UI и фактическая атака, **но без LOF/stuck** (`NoLineOfFire` ставится только в `GetAttackResults`). `AIGetAttackTargetingOptions` для Dump считает CTH через `CalcChanceToHit`, не `GetActionResults` (JAZZ-AI-PERF-003). Перед DumpFire один дешёвый луч (`Jazz_DumpCheapLineOfFire`): непробиваемая скала/плита → не `AIPlayCombatAction`, выход в Disengage (JAZZ-AI-002 / PERF-004). Чистая линия на execute заполняет `hits` целью, иначе Dump CTH-попадание давало 0 урона. Внутри одного AI think сетка aim кэшируется в `context.cth_by_aim_cache` по `(enemy.handle, action.id, max_aim)`. `PredictCTH` больше не вычитает линейный recoil: он строит общий `JAZZ_CTHGetRecoilProfile` и суммирует `JAZZ_CTHGetBulletChance` для той же последовательности пуль. Поэтому отдельная правка AI-точности должна считаться изменением общего CTH-контракта.
...
**Fallback Overwatch aim (JAZZ-AI-OW-001 + 007):** override `AIPlaceFallbackOverwatch` в `Code/AiActions.lua`. Vanilla no-sight path целился random 360° / дверь / лицо союзника. JAZZ: якорь `last_known_enemy_pos` / ближайший enemy. Если якорь сам с LOS (открытое) — сдвиг **1–3** тайла (`JazzAI_SoundOffsetPos`). Иначе цель = **первая проходимая плита по BFS** от якоря (≤8 тайлов / 40 клеток) с LOS — куда враг может выйти в обзор (угол дома, дверь, камень), не стена и не клетка за ней. Ночь/Underground: как OW-001 (lit / night-sight / lit scan ±4). Иначе **`false` (revert)**. Dump в невидимую модельку не идёт. Без RNG на aim. Вызывается из `JAZZ_AIDisengage` и vanilla no-sight bunker fail при `FallbackAction=overwatch`.

**Recontact / FallBack chance (JAZZ-AI-007):** ванильный scout-gate в `SelectArchetype` больше не ставит `Scout_LastLocation` до `PickCustom`. Скаут только если нет видимых, есть last known, не Medic/Deserter/Melee/Regroup/FallBack, и last known дальше **20** тайлов. Линия идёт в пояс **14–20** от звука (не на клетку игрока); до **2** probe (`Flank` / Flanker / aura pusher) могут ближе. Кого игрок видит, а боец игрока нет — stay штрафуется (обязан сдвинуться). Отход: порог dead≥2 и ≥30% — только *допуск*; старт `InteractionRand` со шансом = % потерь на порогах 30/50/70; срыв если у офицера ≥3 живых в 8 тайлах. Не стартовать без `GetNearestEnemy`.
