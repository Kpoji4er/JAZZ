...
В обычном режиме Crosshair скрывает точный CTH и показывает у каждого breakdown modifier разное количество зелёных `+` или красных `−` в зависимости от силы эффекта. При активных modding/debug tools показываются точные проценты.

Целевая модель сохраняет это различие режимов: множители остаются внутренней математикой, без debug игрок продолжает видеть только качественную шкалу `+`/`−`, а debug получает точный итог и процентную разбивку. UI должен использовать тот же runtime pipeline, что `Unit:CalcChanceToHit`; отдельный пересчёт создаст расхождение показанного и фактического шанса.
