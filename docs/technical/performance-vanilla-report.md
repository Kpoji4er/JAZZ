...
13. **Dump / AI targeting LoF (JAZZ-AI-PERF-003 / PERF-004)** — Marauder PickBest `GetActionResults` hung in `PrepareAttackArgs` `GetLoFData` on one body-part ray (waterfall). `AIGetAttackTargetingOptions` now uses `CalcChanceToHit`. Dump execute sets `jazz_ai_dump`: skip `PrepareAttackArgs`/`GetAttackResults` GetLoFData, cheap `ProjectileFly`. **PERF-004:** one cheap terrain + unit-sphere ray fills Dump `hits` on a clear line (M5 AI-vs-AI was 0 damage at 90% CTH); unpenetrable terrain or a body on the line aborts DumpFire. Player firearms unchanged.
