...
3. **`AIUpdateDestLosCache` compact + far-skip + dest-cap (JAZZ-AI-PERF-001)** — compact O(n); dests farther than sight from all enemies skip `CheckLOS`; at most **200** CheckLOS dests (priority stay/important/behavior, then nearest). Enemy range shortlist rolled back (full enemies).
4. **`AIEnumValidDests` OptLoc dest-cap (JAZZ-AI-PERF-001 + PERF-002)** — after CollapsePoints, cap `all_destinations` to **200**. Fill: stay/important/behavior, then **Strategy reserve 48** (high ground / anchors / ring), then nearest **threat**. Hash includes `strategy_kept`. Prevents TakeCover over 2k–3k open-slab dests on M1 without dropping far rooftops.
5. ~~**`AIPolicyTakeCover` far skip**~~ — **reverted** (AI hugged cover without threat-facing without full `GetCoverPercentage`). **2026-08-05:** score at most **8** nearest visible threats per dest (`TAKECOVER_ENEMY_CAP`) — keeps threat-facing, cuts M1 Rebel×Legion OptLoc cost.
6. **`AIPrecalcDamageScore` soft prune + DestLOS early-out + dest-cap (JAZZ-AI-PERF-001)** — range prune only if &gt;12 targets (wide margin); dest with cache `false` без `GetLoFData`; scored dests capped to **48**.
7. **Sight armor integer path** — `GetDegradationMultiplierPermille` вместо float `GetDegradationMultiplier` в `GetSightRadius`.
...
13. **Dump / AI targeting LoF (JAZZ-AI-PERF-003 / PERF-004)** — Marauder PickBest `GetActionResults` hung in `PrepareAttackArgs` `GetLoFData` on one body-part ray (waterfall). `AIGetAttackTargetingOptions` now uses `CalcChanceToHit`. Dump execute sets `jazz_ai_dump`: skip `PrepareAttackArgs`/`GetAttackResults` GetLoFData, cheap `ProjectileFly`. **PERF-004:** cheap terrain + first-3 impassable slabs (entity/slab rock, not a pit) + unit-sphere ray fills Dump `hits` on a clear line (M5 AI-vs-AI was 0 damage at 90% CTH); unpenetrable terrain/rock or a body on the line aborts DumpFire. Dump at a unit model needs team LOS + clear LoF (personal LOS not required; L4 ShockTrooper vs Benny: team vis through mesh rock failed cheap LoF). Player firearms unchanged.
