...
13. **Dump / AI targeting LoF (JAZZ-AI-PERF-003 / PERF-004)** — Marauder PickBest `GetActionResults` hung in `PrepareAttackArgs` `GetLoFData` on one body-part ray (waterfall). `AIGetAttackTargetingOptions` now uses `CalcChanceToHit`. Dump execute sets `jazz_ai_dump`: skip `PrepareAttackArgs`/`GetAttackResults` GetLoFData, cheap `ProjectileFly`. **PERF-004:** one prediction `GetLoFData` (same args as CombatAI zone/dest score) fills Dump `hits` or `stuck`; unpenetrable LoF aborts DumpFire and signature BasicAttack. Dump at a unit model needs team LOS + clear LoF (personal LOS not required). Player firearms unchanged.
