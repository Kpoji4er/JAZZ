...
4. **`AIEnumValidDests` OptLoc dest-cap (JAZZ-AI-PERF-001 follow-up)** — after CollapsePoints, cap `all_destinations` to 400 (stay/important/behavior, then nearest **threat**). Prevents TakeCover over 2k–3k open-slab dests on M1.
5. ~~**`AIPolicyTakeCover` far skip**~~ — **reverted** (AI hugged cover without threat-facing without full `GetCoverPercentage`).
6. **`AIPrecalcDamageScore` soft prune + DestLOS early-out (JAZZ-AI-PERF-001)** — range prune only if &gt;12 targets (wide margin); dest with cache `false` без `GetLoFData`.
