...
Игровая симуляция JA3 зависит от синхронизируемого состояния. Для случайного результата использовать движковые детерминированные функции и подходящий seed/context. Не применять `math.random` в боевой, AI или стратегической логике, если результат влияет на состояние игры.
