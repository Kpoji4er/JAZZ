...
JAZZ поддерживает только последнюю опубликованную CommonLib из официальной ветки `main`/Steam Workshop; совместимость со старыми версиями не заявляется. Перед каждой задачей и каждым поддерживаемым тестом upstream и установленная копия сверяются заново. Перед релизом `version_major`/`version_minor` объявленной зависимости `JA3_CommonLib` также обновляются до текущих значений upstream. На 1 сентября 2026 года последним был CommonLib 1.11, build 1060, commit `6758d82e5fccbf6bdd01b0173d2496b925982c3e`; это снимок проверки, не pin. С июля 2026 (build 1056 / `1adf9f23`) upstream добавил SafeEval, убрал лишний assert и привязал radio pre-game menu к кампании — одноимённых новых коллизий с JAZZ на этом срезе не видно. JA3 проверяет у dependency минимальную пару `version_major.version_minor`; автоматически увеличиваемый revision/build не является частью ограничения зависимости.
...
- `jazz-maps`;
- основной `jazz`;
- без ванильных Advanced Game Rules (JAZZ скрывает всю секцию; базовые Game Rules остаются);
...
COMPAT-011 (2026-08-20): `FortressPierre` / Pierre / PierreGuard keep-vanilla at H4. Spec: [JAZZ-COMPAT-011](../specs/active/JAZZ-COMPAT-011.md). I1 ≠ fort (Pierre is H4, not Flag Hill). Whole-sector I1 skip **reverted** (2026-08-20, owner): empty Flag Hill was RIS↔NoMaps `SpawnObjects` wrap cycle in `System_RIS_Combat.lua` (`lInstallUnitMarkerWrap` re-based onto the nomaps wrap), not class-remap. I1 markers remap as before (`WeakFlagHill`→T1 Roughneck).
