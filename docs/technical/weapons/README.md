...
Принятая [целевая модель стрельбы](accuracy-model.md) исключает `handling` из CTH. Поле удаляется или окончательно помечается legacy только в общей generated-миграции оружия; до неё CSV продолжает точно отражать стабильный runtime baseline.
