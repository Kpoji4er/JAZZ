...
- `JAZZ-AI-ROLE-002-AC-002`: `PASS` — static: Legion Assault/Front/Flanker/Gunner/Heavy/Recruit PickCustom → helper; нет `Hide()` / shadowed panic в companions. **2026-08-18:** editor `items.lua` PickCustom synced (was still vanilla panic/Hide/CQB@8). Gate: `docs/tools/_audit_ai_packet1b.py`. **2026-08-24:** `JazzAI_UnitClassName` читает `unitdatadef_id` (live `class` = `Unit` → все family были Line; ракетчик уходил в `Legion_Regroup` без `RocketLauncherFire`).
