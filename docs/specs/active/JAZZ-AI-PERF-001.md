...
- Guarantee ≤20 s при pathological open slabs × 100 in-range enemies with no geometric prune (follow-up dest caps).
...
JA3Debug required. In console before combat: `config.JAZZ_AIPerfLog = true`. Reproduce M1 large fight; read `[JAZZ-AI-PERF]` lines for DestLos/Precalc/AITurn ms.
