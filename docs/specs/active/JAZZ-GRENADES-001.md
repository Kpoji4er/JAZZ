...
- новый UI «кольцо разброса» на земле (можно follow-up);
...
- `JAZZ-GRENADES-001-REQ-007` — technical + showcase/wiki (если заметно игроку) описывают always-scatter / mishap / distance.

## Инварианты и ограничения

- deterministic MP: без float в bounds `RandRange`; порядок RNG grenade vs heavy согласован на хосте;
- prediction не крутит mishap RNG и не показывает ложный mishap toast;
...
5. **UI scatter preview** — out of scope (рекомендация) / минимальный текст «разброс ~N тайлов» в AP indicator?

## Evidence

- `JAZZ-GRENADES-001-AC-001`–`007`: `BLOCKED` — ожидает approval и реализацию.
