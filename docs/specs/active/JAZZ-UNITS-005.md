...
- `JAZZ-UNITS-005-REQ-019` — appearance: **уникальный клон на слот** `JAZZ_AME_01`…`60` (`ModItemAppearancePreset`, group `AME`). **Канон Legion-одежды** = handcrafted jazz-units `Legion*` (`LegionGoon`/…; **не** vanilla `Legion_*` как preferred donor). Пулы: jazz `Legion*` / Rebels / GrandChien / keep; **Irregulars** lean jazz `Legion*`. Карта: [`ame-appearance-map.json`](../../design/ame-appearance-map.json) (`donor`, `head_swap`, ♀ `hair`). Clothing patch preserves Head + BodyColor C1 + HeadColor. **Ровно один AME-синий акцент на одежде** (Shirt → BodyC2 → Chest → Armor → Hip; **не** Pants; **не** Hat/Hat2). Legion-red / лишний blue → slate. **Голова:** Af bank only; без `Faction_Legion_Head_*`. **♀ Hair:** только `NPCFemale_Hair_*` (не AIM `Equipment*_Hair`); если Hat/Hat2 занят → `Hair=""`. **Headwear:** снимать каски/тюрбаны/`FactionMale_Hat_*`/`Equipment*_Hat`/`NPCCostumeFemale_Hat*`; оставлять mask/scarf/glasses/headband. Исходные donor presets **не править**. Tooling: `_patch_ame_appearance_clothes_from_map.py`. Policy: [`ame-appearance-assets.md`](../../design/ame-appearance-assets.md).
