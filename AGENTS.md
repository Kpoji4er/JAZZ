...
7. `docs/technical/` описывает текущее состояние для разработчика, `docs/wiki/` — текущее состояние для игрока, а `docs/specs/` — утверждённое намерение; затронутые уровни документации входят в DoD.
