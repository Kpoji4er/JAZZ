UndefineClass('ShapedCharge')
DefineClass.ShapedCharge = {
	__parents = { "GrenadeItem" },
	__generated_by_class = "ModItemInventoryItemCompositeDef",


	object_class = "GrenadeItem",
	Repairable = false,
	Reliability = 100,
	Icon = "UI/Icons/Weapons/ShapedCharge",
	ItemType = "Grenade",
	DisplayName = T(282820475304, "Кумулятивный заряд"),
	DisplayNamePlural = T(749789148569, "Кумулятивные заряды"),
	AdditionalHint = T(622419236578, "<image UI/Conversation/T_Dialogue_IconBackgroundCircle.tga 400 130 128 120> Наносит урон по сектору\n<image UI/Conversation/T_Dialogue_IconBackgroundCircle.tga 400 130 128 120> Высокая вероятность неудачи при использовании всеми, кроме Барри"),
	UnitStat = "Explosives",
	Cost = 1500,
	CanAppearInShop = true,
	RestockWeight = 0,
	MaxStacks = 2,
	MinMishapChance = 2,
	MaxMishapChance = 60,
	MaxMishapRange = 6,
	CenterObjDamageMod = 300,
	AreaOfEffect = 8,
	CenterAreaOfEffect = 4,
	AreaUnitDamageMod = 50,
	PenetrationClass = 1,
	coneShaped = true,
	coneAngle = 45,
	BurnGround = false,
	DeathType = "BlowUp",
	BaseDamage = 80,
	Scatter = 6,
	BaseRange = 0,
	ThrowMaxRange = 23,
	CanBounce = false,
	Noise = 100,
	Entity = "MilitaryCamp_Grenade_01",
	ActionIcon = "UI/Icons/Hud/shaped_charge",
}

